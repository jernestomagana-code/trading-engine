...
FAST_WATCHLIST = [
    "QQQ", "SPY", "AAPL", "NVDA", "TSLA", "NFLX",
    "META", "AMZN", "MSFT", "GOOGL", "TLT", "RSP",
]
FAST_OPTION_SYMBOLS = list(DEFAULT_OPTION_SYMBOLS)

WATCHLIST_REQUESTED = _env_csv_list(
    "IBKR_WATCHLIST",
    ["RSP"] if COBERTURAS_RSP_WEEKLY else (FAST_WATCHLIST if DAILY_RADAR_FAST else DEFAULT_WATCHLIST),
)
MAX_WATCHLIST_SYMBOLS_PER_RUN = max(1, _env_int(
    "IBKR_MAX_WATCHLIST_SYMBOLS_PER_RUN",
    1 if COBERTURAS_RSP_WEEKLY else (12 if DAILY_RADAR_FAST else len(DEFAULT_WATCHLIST)),
))
WATCHLIST = WATCHLIST_REQUESTED[:MAX_WATCHLIST_SYMBOLS_PER_RUN]
...
print(
    ...
    f" | watchlist_budget:{len(WATCHLIST)}/{len(WATCHLIST_REQUESTED)}"
