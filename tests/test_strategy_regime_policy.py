...
class StrategyRegimePolicyTests(unittest.TestCase):
    ...
    def test_parameter_review_blocks_when_avoid_if_condition_is_triggered(self):
        overlay = strategy_regime_policy.regime_overlay("COVERED_CALL", "NEUTRAL_RANGE", self.policy)
        item = {
            "final_state": "ENTRY_READY",
            "blockers": ["EX_DIVIDEND_WITHIN_WINDOW"],
            "required_missing_fields": [],
            "evidence": {
                "technical": {"score": 82},
                "fundamental": {},
                "options": {"contract": {"delta": 0.20, "dte": 33, "spread_pct": 11.5}},
                "market": {"ex_dividend_soon": True, "assignment_acceptable": True},
            },
        }
