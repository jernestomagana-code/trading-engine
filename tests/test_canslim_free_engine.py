...
class CanslimFreeEngineTests(unittest.TestCase):
    ...
    def test_scores_companyfacts_and_runtime_relative_strength(self):
        ...
        self.assertEqual(row["canslim_component_coverage_pct"], 100.0)
        self.assertIsNotNone(row["canslim"]["components"]["L_relative_strength"])
        self.assertIsNotNone(row["canslim"]["components"]["M_market"])
        self.assertEqual(row["canslim_scope"], "FULL_C_A_L_M")
        self.assertEqual(payload["historical_data_health"]["status"], "FULL")
        self.assertTrue(payload["historical_data_health"]["spy_available"])
        self.assertTrue(payload["historical_data_health"]["qqq_available"])
