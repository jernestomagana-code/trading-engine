#!/usr/bin/env python3
"""Check whether real TradingView alerts have completed the E2E loop."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Check real TradingView end-to-end readiness.")
    parser.add_argument("--runtime-dir", default="runtime")
    parser.add_argument("--coverage", default="config/tradingview_alert_coverage_v1.json")
    parser.add_argument("--output", default="runtime/tradingview_e2e_readiness.json")
    parser.add_argument("--no-write", action="store_true")
    parser.add_argument("--market-closed-ok", action="store_true")
    parser.add_argument(
        "--local-replay-validation",
        action="store_true",
        help="Validate normalization locally without writing synthetic events to runtime.",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = tradingview_operational_health.build_e2e_readiness(
        args.runtime_dir,
