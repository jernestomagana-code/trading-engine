            ...
            for item in candidates:
                try:
                    ...
                    def add_missing(name):
                        if name not in missing_confirmations:
                            missing_confirmations.append(name)

                    def is_finite_number(value):
                        return isinstance(value, (int, float)) and not isinstance(value, bool) and math.isfinite(value)

                    if not is_finite_number(contract.strike) or contract.strike <= 0:
                        add_missing("strike")
                    if not contract.lastTradeDateOrContractMonth:
                        add_missing("expiration")
                    if not is_finite_number(dte) or dte < 0:
                        add_missing("dte")
                    if not is_finite_number(bid) or bid <= 0:
                        add_missing("bid")
                    if not is_finite_number(ask) or ask <= 0:
                        add_missing("ask")
                    if is_finite_number(bid) and is_finite_number(ask) and ask < bid:
                        add_missing("bid_ask_order")
                    if not is_finite_number(mid) or mid <= 0:
                        add_missing("mid")
                    if not is_finite_number(spread) or spread < 0:
                        add_missing("spread")
                    if not is_finite_number(spread_pct) or spread_pct < 0:
                        add_missing("spread_pct")
                    delta = greeks.get("delta")
                    if not is_finite_number(delta) or delta < -1 or delta > 1:
                        add_missing("delta")

                    execution_data_complete = len(missing_confirmations) == 0

                    # The bridge can confirm executable contract data, but it cannot
                    # confirm TradingView technicals, cloud risk gates, or manual blockers.
                    ...
                    payload = {
                        ...
                        "recommendation": "VALIDAR_TECNICO_RIESGO_MANUAL_EN_CLOUD" if execution_data_complete else "ESPERAR_DATOS_EJECUTABLES",
                        "reason": reason,
                        "v30_contract_enrichment": True,
                        "v30_required_fields_complete": execution_data_complete,
                        "v30_execution_data_complete": execution_data_complete,
