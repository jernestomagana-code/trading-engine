...
El calendario automático utiliza **Alpha Vantage Earnings Calendar** como fuente gratuita principal. La apertura diaria consulta los candidatos CANSLIM completos y conserva cada fecha con su fuente y momento de observación. Las fechas futuras comienzan como `ESTIMATED`: permiten investigación y vigilancia de volatilidad, pero deben corroborarse antes de cualquier uso operativo. La ausencia de Wall Street Horizon nunca bloquea esta estrategia.

La clave gratuita se lee desde `ALPHA_VANTAGE_API_KEY` o desde Keychain con el servicio `stock-ultimus-alpha-vantage-api-key`; nunca se escribe en runtime ni se muestra en la consola. TradingView y la página de relaciones con inversionistas sirven para corroboración. Wall Street Horizon permanece como respaldo opcional si en el futuro se contrata. La rejilla SPY/RSP sólo se considera completa cuando cada combinación de activo y horizonte tiene al menos un contrato con interés abierto mínimo 500, spread máximo 8% y delta suficientemente cercana al objetivo.

Configuración inicial segura: obtener una clave gratuita en `https://www.alphavantage.co/support/#api-key` y ejecutar `python3 scripts/setup_alpha_vantage_key.py`. El ingreso es oculto y la clave queda únicamente en Keychain.
