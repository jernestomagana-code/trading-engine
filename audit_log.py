    ...
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            if is_sensitive_key(key):
                ...
                result[str(key)] = redact(item, max_depth=max_depth - 1)
        return result
...
def write_events(events: list[dict[str, Any]], path: str | Path = DEFAULT_AUDIT_LOG_PATH, *, max_events: int = MAX_AUDIT_EVENTS) -> None:
    p = Path(path)
