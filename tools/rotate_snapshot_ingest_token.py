#!/usr/bin/env python3
"""Rotate the local Stock Ultimus snapshot ingest token safely.

The token is never printed. By default this stores a new random value in the
macOS Keychain service used by the local bridge. Use --copy to place it on the
clipboard so it can be pasted into Render as SNAPSHOT_INGEST_TOKEN.
"""
...
DEFAULT_KEYCHAIN_SERVICE = "stock-ultimus-snapshot-ingest"
...
def rotate_token(service: str, account: str, *, copy: bool = False) -> dict[str, object]:
    ...
    return {
        ...
        "token_printed": False,
        "next_required_action": "Paste the clipboard value into Render SNAPSHOT_INGEST_TOKEN, then redeploy/retry bridge publish.",
    }


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Rotate local Stock Ultimus snapshot ingest token without printing it.")
...
def main() -> int:
    ...
    print(
        "Snapshot ingest token rotated locally. "
