...
class V31CanonicalDecisionTests(unittest.TestCase):
    ...
    def test_v32_intraday_futures_immediate_notify_sends_and_dedupes_entry_event(self):
        ...
        self.assertTrue(result["would_notify"])
        self.assertTrue(result["pushover_sent"])
        self.assertIn("MNQ1! LONG", result["message"])
        self.assertIn("Disparo: 100.00", result["message"])
        self.assertIn("esperar actualización", result["message"])
        self.assertFalse(result["execution_authorized"])
        self.assertTrue(result["not_order_instruction"])
        send_push.assert_called_once()
        save_state.assert_called_once()
    ...
    def test_v32_intraday_futures_immediate_notify_sends_manual_review_entry_trigger(self):
        ...
        self.assertTrue(result["would_notify"])
        self.assertTrue(result["pushover_sent"])
        self.assertIn("revisar ahora; aún no ejecutar", result["message"])
        self.assertNotIn("MANUAL_REVIEW", result["message"])
        self.assertNotIn("RISK_ENGINE_NEEDS_REVIEW", result["message"])
        self.assertFalse(result["execution_authorized"])
        self.assertTrue(result["not_order_instruction"])
        send_push.assert_called_once()
