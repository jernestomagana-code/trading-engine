...
- **Radar CANSLIM:** presenta un embudo operativo ordenado por cercanía a una decisión, no por el score aislado: **Universo → Preselección → Contrato → Setup/compuerta → Entrada lista**. También informa cuántas lecturas son CANSLIM completas y cuántas siguen siendo preliminares. Los cinco candidatos más cercanos a una decisión forman la lista principal del día; el resto queda plegado. Cada tarjeta responde primero **por qué está en la lista**, **entrada esperada**, **invalidación** y **dato pendiente**. Después muestra cobertura C/A/L/M, gatillo, fortaleza relativa, punto de compra/distancia, volumen, estrategia propuesta, bloqueo principal y el siguiente evento que debe ocurrir. Un precio de una evaluación anterior se identifica como referencia y nunca como entrada vigente. Cuando la fuente todavía no entrega una métrica —actualmente pueden faltar L, M, punto de compra, soporte de invalidación o volumen relativo— aparece como **Pendiente/no disponible**; la consola no la supone. Si faltan L o M, el score se rotula como parcial. Una preselección CANSLIM no equivale a una recomendación de compra.
...
Cuando IBKR detecta una posición, la consola busca automáticamente la recomendación reciente que pudo originarla. Sólo muestra **Recomendación vinculada** cuando existe evidencia adicional al ticker —por ejemplo estrategia, familia de futuro, strike, vencimiento o lado— y presenta el grado de confianza. Una coincidencia únicamente por ticker aparece como **Coincidencia candidata; no confirmada**. El plan posterior compara la entrada detectada por IBKR con la entrada recomendada y recupera invalidación/stop, objetivos, próxima revisión, origen e identificador. Si no existe evidencia suficiente, muestra **Sin recomendación vinculable** y continúa administrando la posición con el plan actual del motor; nunca inventa la atribución. Este vínculo es informativo y no demuestra que la ejecución haya seguido exactamente la señal.
...
Cuando existen acciones y calls vendidas del mismo activo, el motor no las interpreta como exposiciones aisladas. Suma los lotes de 100 acciones y todas las calls cortas, calcula la cobertura agregada y muestra **Covered call completo reconocido** o **Covered call parcial reconocido**, junto con cantidad de acciones, contratos, strikes, vencimientos, porcentaje cubierto y capacidad real para calls nuevas. Si toda la posición ya está cubierta, la consola presenta una sola estructura visible y coloca la gestión de las acciones dentro de esa tarjeta; no vuelve a recomendar calls nuevas. Si las calls vendidas exceden los lotes disponibles, señala el exceso como riesgo descubierto: unas mismas 100 acciones nunca pueden respaldar dos calls distintas simultáneamente.

Para calcular captura de prima, IBKR puede entregar `averageCost` de una opción como valor por contrato mientras el mark está expresado por acción. La consola normaliza automáticamente ese costo dividiéndolo por el multiplicador —normalmente 100— antes de decidir si existe ganancia suficiente para recomprar; así evita tomar una pérdida como una captura de prima positiva.
...
La Torre de Control consulta directamente `positions()` de IBKR y su lectura vigente es autoritativa frente a recuerdos, eventos históricos o snapshots antiguos. Si una posición como MNQ aparece con cuenta, vencimiento y cantidad en **Posiciones activas**, debe verificarse o cerrarse manualmente en TWS; marcarla como revisada en la consola sólo registra la revisión y no altera la posición del broker.
